 ...
 */
async function makeAuthenticatedVelocityCall(
  endpoint: string,
  method: 'GET' | 'POST',
  context: any,
  body?: any
  ...
  if (!cookies) {
    console.warn('No authentication context provided to velocity tool');
  ...
  try {
    ...
    console.error('Error in makeAuthenticatedVelocityCall:', error);
...
export const getSprintVelocityTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    try {
      ...
      const response = await makeAuthenticatedVelocityCall(
        `/velocity/sprint/${validated.sprint_id}/velocity`,
        'GET',
...
export const getProjectAverageVelocityTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    try {
      ...
      if (validated.exclude_sprint_id) params.append('exclude_sprint_id', validated.exclude_sprint_id.toString());

      // Call the API to get average velocity
      const response = await makeAuthenticatedVelocityCall(
        `/velocity/project/${validated.project_id}/velocity/average?${params.toString()}`,
        'GET',
        experimental_context
      );
...
export const getProjectVelocityMetricsTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    try {
      ...
      const response = await makeAuthenticatedVelocityCall(
        `/velocity/project/${validated.project_id}/velocity/metrics`,
        'GET',
        experimental_context
      );
...
export const getProjectVelocityTrendTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    try {
      ...
      const response = await makeAuthenticatedVelocityCall(
        `/velocity/project/${validated.project_id}/velocity/trend?limit=${validated.limit}`,
        'GET',
        experimental_context
      );
