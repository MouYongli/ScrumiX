...
import {
  ...
  backlogRetrievalSchema,
  updateBacklogItemSchema,
  deleteBacklogItemSchema
} from '@/lib/tools/schemas/backlog';
...
const createBacklogItemTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    try {
      ...
      const validated = createBacklogItemSchema.parse(input);
      ...
      // Extract project ID from context or validated input (backend doesn't return project_id in response)
      const projectId = validated.project_id || (experimental_context as any)?.projectId;
...
You can view this ${createdBacklog.item_type} in the [Product Backlog](http://localhost:3000/project/${projectId}/backlog). If you have any further requirements or modifications, just let me know!`;
