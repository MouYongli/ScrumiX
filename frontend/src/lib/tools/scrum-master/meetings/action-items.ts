  ...
  execute: async (input, { experimental_context }) => {
    ...
    switch (validated.operation) {
      case 'create': {
        ...
        const response = await makeAuthenticatedRequest('/meeting-action-items/', { method: 'POST', body: JSON.stringify({ meeting_id: meetingId, title: validated.title, due_date: validated.due_date }) }, experimental_context);
      ...
      case 'read': {
        const response = await makeAuthenticatedRequest(`/meeting-action-items/?meeting_id=${meetingId}`, { method: 'GET' }, experimental_context);
      ...
      case 'update': {
        ...
        const response = await makeAuthenticatedRequest(`/meeting-action-items/${validated.action_item_id}`, { method: 'PUT', body: JSON.stringify({ title: validated.title, due_date: validated.due_date, completed: validated.completed }) }, experimental_context);
      ...
      case 'delete': {
        ...
        const response = await makeAuthenticatedRequest(`/meeting-action-items/${validated.action_item_id}`, { method: 'DELETE' }, experimental_context);
      ...
      case 'complete': {
        ...
        const response = await makeAuthenticatedRequest(`/meeting-action-items/${validated.action_item_id}`, { method: 'PUT', body: JSON.stringify({ completed: true }) }, experimental_context);
