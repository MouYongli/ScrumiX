...
export const manageMeetingAgendaTool = tool({
  ...
  execute: async (input, { experimental_context }) => {
    ...
    if (!meetingId && validated.meeting_search) {
      const res = await makeAuthenticatedRequest(`/meetings/?search=${encodeURIComponent(validated.meeting_search)}&limit=5`, { method: 'GET' }, experimental_context);
    ...
    switch (validated.operation) {
      case 'create': {
        if (validated.agenda_items && validated.agenda_items.length > 0) {
          const response = await makeAuthenticatedRequest(`/meeting-agendas/meeting/${meetingId}/bulk`, { method: 'POST', body: JSON.stringify(validated.agenda_items) }, experimental_context);
        ...
        if (validated.title) {
          const response = await makeAuthenticatedRequest('/meeting-agendas/', { method: 'POST', body: JSON.stringify({ meeting_id: meetingId, title: validated.title, order_index: validated.order_index }) }, experimental_context);
      ...
      case 'read': {
        const readResponse = await makeAuthenticatedRequest(`/meeting-agendas/meeting/${meetingId}`, { method: 'GET' }, experimental_context);
      ...
      case 'update': {
        ...
        const response = await makeAuthenticatedRequest(`/meeting-agendas/${validated.agenda_id}`, { method: 'PUT', body: JSON.stringify({ title: validated.title, order_index: validated.order_index }) }, experimental_context);
      ...
      case 'delete': {
        ...
        const response = await makeAuthenticatedRequest(`/meeting-agendas/${validated.agenda_id}`, { method: 'DELETE' }, experimental_context);
      ...
      case 'reorder': {
        ...
        const response = await makeAuthenticatedRequest('/meeting-agendas/reorder', { method: 'POST', body: JSON.stringify({ agenda_ids: validated.agenda_ids }) }, experimental_context);
