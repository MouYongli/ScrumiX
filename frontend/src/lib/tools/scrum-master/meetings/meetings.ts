  ...
  execute: async (input, { experimental_context }) => {
    ...
    switch (validated.operation) {
      case 'create': {
        ...
        const createResponse = await makeAuthenticatedRequest('/meetings/', { method: 'POST', body: JSON.stringify(createData) }, experimental_context);
...
ID: #${createdMeeting.id} | Type: ${validated.meeting_type || 'other'} | ${tz.displayDateTime}`;
      ...
      case 'read': {
        ...
        const readResponse = await makeAuthenticatedRequest(`/meetings/${validated.meeting_id}`, { method: 'GET' }, experimental_context);
        ...
        const participantsResponse = await makeAuthenticatedRequest(`/meeting-participants/meeting/${validated.meeting_id}`, { method: 'GET' }, experimental_context);
...
**Last Updated:** ${meetingUpdatedAt ? new Date(meetingUpdatedAt).toLocaleString() : 'Unknown'}`;
      ...
      case 'update': {
        ...
        const updateResponse = await makeAuthenticatedRequest(`/meetings/${validated.meeting_id}`, { method: 'PUT', body: JSON.stringify(updateData) }, experimental_context);
...
The meeting details have been saved and participants will see the updated information.`;
      ...
      case 'delete': {
        ...
        const meetingToDeleteResponse = await makeAuthenticatedRequest(`/meetings/${validated.meeting_id}`, { method: 'GET' }, experimental_context);
        ...
        const deleteResponse = await makeAuthenticatedRequest(`/meetings/${validated.meeting_id}`, { method: 'DELETE' }, experimental_context);
...
⚠️ **Note:** This action cannot be undone. All associated agenda items, action items, and notes have also been removed.`;
      ...
      case 'list': {
        ...
        const listResponse = await makeAuthenticatedRequest(`/meetings/?${params.toString()}`, { method: 'GET' }, experimental_context);
