...
async def update_task_embeddings_background(task_id: int, db: Session):
    """Background task to update embeddings for task"""
    try:
...
def update_task(
    ...
    # Check if content changed and schedule embedding update
    content_changed = False
    if task_in.title and task_in.title != old_title:
        content_changed = True
    if task_in.description is not None and task_in.description != old_description:
        content_changed = True
    if task_in.status and task_in.status != old_status:
        content_changed = True
    if task_in.priority and task_in.priority != db_task.priority:
        content_changed = True
    
    if content_changed:
        schedule_embedding_update(background_tasks, task_id, db)
...
async def semantic_search_tasks(
    ...
    """
    Perform semantic search on tasks using embeddings.
    Searches based on title, description, status, and priority content.
    """
    try:
...
async def find_similar_tasks(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Similar tasks search failed: {str(e)}"
        )
